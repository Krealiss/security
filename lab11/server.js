// Camp Booking Validator — Labs 9, 10 & 11
// Usage:
//   node server.js                           -> vulnerable (Lab 9/10)
//   MODE=validated node server.js            -> validated (Lab 9 Task 4)
//   MODE=validated-sql node server.js        -> safe SQL  (Lab 10 Task 2)
//   MODE=clickjack-vulnerable node server.js -> no frame protection  (Lab 11 Task 1)
//   MODE=clickjack-csp node server.js        -> CSP frame-ancestors 'none' (Lab 11 Task 2)
//   MODE=clickjack-csp-self node server.js   -> CSP frame-ancestors 'self' (Lab 11 Task 2)
//   MODE=clickjack-xfo node server.js        -> X-Frame-Options: DENY   (Lab 11 Task 3)
//   PORT=3000 node server.js                 -> override listen port
...
// Lab 11: frame protection middleware
if (MODE.startsWith('clickjack-') && MODE !== 'clickjack-vulnerable') {
  app.use((req, res, next) => {
    if (MODE === 'clickjack-csp') {
      res.setHeader('Content-Security-Policy', "frame-ancestors 'none'");
      console.log("[Frame] CSP frame-ancestors 'none' set");
    } else if (MODE === 'clickjack-csp-self') {
      res.setHeader('Content-Security-Policy', "frame-ancestors 'self'");
      console.log("[Frame] CSP frame-ancestors 'self' set");
    } else if (MODE === 'clickjack-xfo') {
      res.setHeader('X-Frame-Options', 'DENY');
      console.log("[Frame] X-Frame-Options: DENY set");
    }
    next();
  });
}
...
// Lab 11: sensitive "confirm payment" page (clickjack target)
app.get('/confirm', (req, res) => {
  res.send(layout('Confirm Payment', `
    <p style="color:#6b7280;font-size:14px">Review your booking before confirming.</p>
    <div style="background:#f9fafb;border:1px solid #e5e7eb;border-radius:10px;padding:20px;margin-bottom:20px">
      <strong>Camp Summer 2027</strong><br>
      <span style="font-size:14px;color:#6b7280">July 15 – July 22 • 480 UAH</span>
    </div>
    <form method="POST" action="/confirm">
      <button type="submit" id="confirm-btn"
        style="width:100%;padding:14px;background:#16a34a;color:#fff;border:none;border-radius:8px;font-size:16px;font-weight:700;cursor:pointer">
        ✅ Confirm &amp; Pay
      </button>
    </form>
    <p style="font-size:12px;color:#9ca3af;margin-top:10px">
      Mode: ${MODE} — ${MODE==='clickjack-vulnerable'?'⚠️ No frame protection!':'Frame protection active.'}
    </p>`));
});

app.post('/confirm', (req, res) => {
  console.log('[PAYMENT] Confirm & Pay clicked — booking confirmed!');
  res.send(layout('Payment Done', `
    <div class="ok">💳 Payment confirmed! (clickjack succeeded if you did not see this page)</div>
    <a href="/confirm">← Back</a>`));
});
