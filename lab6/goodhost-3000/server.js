...
// session store: sid -> { username, createdAt }
const sessions = {};
const SESSION_TTL = 2 * 60 * 1000; // 2 min
const isLifecycleSecure = config.mode === 'lifecycle-secure';

// TTL cleanup runs every 30s in lifecycle-secure mode
...
app.get('/login', (req, res) => {
  ...
  sessions[user.sid] = { username, createdAt: Date.now() };
...
// server-side logout: removes session from store
...
// /api/emails: in lifecycle-secure, requires valid session
app.get('/api/emails', (req, res) => {
  if (isLifecycleSecure) {
    const sid = getSessionId(req);
    if (!isSessionValid(sid)) {
      console.log(`[Auth] /api/emails rejected: invalid session (sid=${sid})`);
