// GoodHost - main WebMail app
// HTTP  -> http://localhost:3000  (redirect to HTTPS in tls mode, normal otherwise)
// HTTPS -> https://localhost:3443 (only in tls mode)
//
// config.json "mode":
...
//   csrf-vulnerable   - GET delete, no SameSite, no token
//   csrf-samesite-lax - GET delete, SameSite=Lax
//   csrf-samesite-strict - GET delete, SameSite=Strict
//   csrf-token        - POST delete, requires x-csrf-token header
//   tls               - HTTPS on :3443, HTTP :3000 redirects, HSTS, Secure+SameSite cookie
...
const isTLS = false; // overridden below after config load
...
const isTLSMode = config.mode === 'tls';
...
// HSTS header for all responses in tls mode
if (isTLSMode) {
...
function cookieFlags() {
  ...
  if (config.mode === 'tls')                   return '; Path=/; HttpOnly; Secure; SameSite=Strict';
  return '; Path=/';
}
...
app.get('/api/logout', (req, res) => {
  ...
  if (sid && sessions[sid]) {
    console.log(`[Auth] logout: ${sessions[sid].username} (${sid})`);
    delete sessions[sid];
  }
...
// GET delete (Tasks 1-3 Lab 7: CSRF demo)
app.get('/api/emails/delete/:id', (req, res) => {
  ...
  console.log(`[DELETE GET] id=${id} deleted=${before !== emails.length}`);
  res.json({ ok: before !== emails.length, id, remaining: emails.length });
});

// POST delete with CSRF token (Task 4 Lab 7)
app.post('/api/emails/delete/:id', (req, res) => {
  ...
  if (config.mode === 'csrf-token' && token !== expected) {
    console.log(`[DELETE POST] CSRF rejected: got="${token}"`);
    return res.status(403).json({ error: 'Forbidden - invalid CSRF token' });
  }
  const id = parseInt(req.params.id, 10);
  const before = emails.length;
  emails = emails.filter(e => e.id !== id);
  console.log(`[DELETE POST] id=${id} deleted=${before !== emails.length} token OK`);
  res.json({ ok: before !== emails.length, id, remaining: emails.length });
});
...
app.get('/other', (req, res) => {
  const sid = getSessionId(req);
  res.json({ path: '/other', cookieReceived: !!sid });
});
...
if (isTLSMode) {
  ...
  https.createServer(tlsOptions, app).listen(PORT_HTTPS, () => {
    console.log(`[System] HTTPS server listening on https://localhost:${PORT_HTTPS}`);
  });

  // HTTP on 3000: 301 redirect to HTTPS
  http.createServer((req, res) => {
    const location = `https://localhost:${PORT_HTTPS}${req.url}`;
    console.log(`[Redirect] ${req.url} -> ${location}`);
    res.writeHead(301, { Location: location });
    res.end();
  }).listen(PORT_HTTP, () => {
    console.log(`[System] HTTP redirect listening on http://localhost:${PORT_HTTP}`);
  });
} else {
  // plain HTTP for all previous labs
  http.createServer(app).listen(PORT_HTTP, () => {
    console.log(`[System] GoodHost listening on http://localhost:${PORT_HTTP}`);
  });
}
